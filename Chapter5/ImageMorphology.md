...
Последние два оператора **Top Hat** и **Black Hat** используются для изоляции более ярких и более темных регионов, чем их соседи. Данные операторы могут быть использованы для изоляции частей объекта, которые проявляют изменение яркости по отношению к объекту, к которому они присоединены. Это зачастую всчтречается, например, при обработки изображений клеток под микроскопом. Обе операции основаны на более примитивных операторах: 
