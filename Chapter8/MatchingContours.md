...
Иногда уже при наличии контура, не известно является ли он выпуклым. В этом случае можно воспользоваться функцией *cvCheckContourConvexity()*. Тестирование данной функцией выполняется просто и быстро (на самом деле это занимает O(N) времени, что немного быстрее, чем O(N logN) времени, необходимого для построения каркаса выпуклости), однако данная функция будет некорректно работать, если контур содержит самопересечения.
