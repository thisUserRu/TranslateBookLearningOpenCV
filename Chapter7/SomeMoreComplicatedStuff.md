...
Если снова взглянуть на рисунок 7-4, то можно увидеть характер изменения расстояния EMD. Точное совпадение соответствует расстоянию равное 0. Совпадение на половину это половина "полного перебора" - то количество, которое потребуется, чтобы перенести половину левой гистограммы в соседний слот. И наконец, для перемещения всей гистограммы на один шаг вправо потребует вся единица расстояния (например, для изменения гистограммы в "совершенно несовпадающую " гистограмму).
...
Шаблон соответствия, используемый в *cvMatchTemplate()* основано не на гистограммах; функция сравнивает текущее изображение *patch* с исходным изображением при помощи "скольжения" *patch* над исходным изображением, при использовании одного из методов сравнения, описанных в конце данного раздела.
