...
Теперь, когда уже есть знания по загрузке и отображении изображения, а также обработке событий от клавиш клавиатуры, можно перейти к обработке событий мыши.
...
В HighGUI ползунок именуется trackbar-ом. Это все потому, что в оригинальной (исторической) трактовке данный элемент используется для выбора конкретного кадра из видеопотока. При помощи данного элемента можно делать как привычные вещи, связанные с ползунком, так и более необычные (будут рассмотрены в следующем разделе "No Buttons"). 
...
*Switches* (переключатель) это облегченный вариант слайдера, у которого только две позиции "on" (1) и "off" (0) (*count = 1*). Используя слайдер как кнопку, функция обратного вызова автоматически сбрасывает переключатели в 0 (пример 4-2) или же автоматически сбрасывает в 0 другие переключателей (эффект "радиокнопок").
