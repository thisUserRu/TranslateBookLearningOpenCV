...
Предположим, имеется один объект *codeBook* с *code_elements* для каждого пикселя. Более того, необходим массив *codeBook*, который по длине равен числу пикселей изображения. Для каждого пикселя вызывается функция *update_codebook()* вызывается ровным счетом столько раз, чтобы охватить соответствующие изменения в фоне. Процесс обучения может периодически обновляться, а функция *clear_stale_entries()* может быть использована для обучения модели фона в присутствии (небольшого числа) двигающихся объектов. Это становиться возможным за счет удаления редко используемых "устаревших" записей, созданных благодаря движущимся объектам. Реализация *update_codebook()* представлена далее:
...
В этой главе были представлены два метода моделирования фона: метод усреднения и метод кодовой книги. Возможно, будет интересно узнать, какой  из методов лучше или, по крайней мере, какой проще в использовании. В таком случае лучше всего сравнить имеющиеся алгоритмы между собой.
